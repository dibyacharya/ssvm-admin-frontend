...
import {
  ...
  AlertTriangle, MessageSquare, TicketCheck,
} from "lucide-react";
import * as svc from "../services/helpdeskConfig.service";
import RequestTypeConfig from "./RequestTypeConfig";
...
export default function HelpdeskConfig() {
  ...
  const tabs = [
    { id: "queries", label: "Query Configuration", icon: MessageSquare },
    { id: "service-requests", label: "Service Requests", icon: TicketCheck },
  ];
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Manage query categories, service requests, and role-based routing
        </p>
      </div>
                        ...
                        categories.map((cat, idx) => (
                          <tr key={cat._id} className={`hover:bg-gray-50 ${!cat.isActive ? "opacity-50" : ""}`}>
                            ...
                            <td className="px-4 py-3 font-medium text-gray-900">
                              {cat.name}
                              {cat.special && (
                                <span className="ml-2 px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full">{cat.special}</span>
                              )}
                            </td>
                            <td className="px-4 py-3 text-gray-600 max-w-xs">
                              <div className="flex flex-wrap gap-1">
                                {(cat.subCategories || []).slice(0, 3).map((sc, i) => (
                                  <span key={i} className="px-2 py-0.5 bg-gray-100 text-gray-700 text-xs rounded">{sc}</span>
                                ))}
                                {(cat.subCategories || []).length > 3 && (
                                  <span className="px-2 py-0.5 bg-gray-200 text-gray-600 text-xs rounded">+{cat.subCategories.length - 3} more</span>
                                )}
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-3">
                              <button onClick={() => handleToggleActive(cat)}
                                className={`px-2 py-1 text-xs rounded-full font-medium ${cat.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                                {cat.isActive ? "Active" : "Inactive"}
                              </button>
                            </td>
                            <td className="px-4 py-3 text-right">
                              <div className="flex items-center justify-end gap-2">
                                <button onClick={() => openEdit(cat)} className="p-1.5 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded" title="Edit">
                                  <Pencil className="w-4 h-4" />
                                </button>
                                <button onClick={() => handleDelete(cat)} className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded" title="Delete">
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
              <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4">
                <div className="flex items-center justify-between px-6 py-4 border-b">
                  <h2 className="text-lg font-semibold">{editingCat ? "Edit Query" : "Add New Query"}</h2>
                ...
                <div className="px-6 py-4 space-y-5">
                  ...
                  {/* Sub-Categories */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Sub-Queries</label>
                    <div className="flex gap-2 mb-2">
                      <input type="text" value={newSubCat}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {form.subCategories.map((sc, idx) => (
                        <span key={idx} className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded-full border border-blue-200">
                          {sc}
                          <button type="button" onClick={() => removeSubCategory(idx)} className="hover:text-red-500"><X className="w-3 h-3" /></button>
                        </span>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="border rounded-lg p-4 bg-gray-50">
                    <h3 className="text-sm font-semibold text-gray-700 mb-3">Role-Based Routing</h3>
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        { key: "l1", label: "L1 (Frontline)", color: "text-blue-700" },
                        { key: "l2", label: "L2 (Escalation)", color: "text-orange-700" },
                        { key: "l3", label: "L3 (Final)", color: "text-red-700" },
                      ].map(({ key, label, color }) => (
                        <div key={key} className="space-y-2">
                          <div className={`text-xs font-bold uppercase ${color}`}>{label}</div>
                          <select value={form.routing[`${key}Role`]}
                            onChange={(e) => setForm((f) => ({ ...f, routing: { ...f.routing, [`${key}Role`]: e.target.value } }))}
                            className="w-full border rounded px-2 py-1.5 text-xs">
                            <option value="">Select Role</option>
                            {roles.map((r) => <option key={r} value={r}>{r}</option>)}
                          </select>
                          <select value={form.routing[`${key}Level`]}
                            onChange={(e) => setForm((f) => ({ ...f, routing: { ...f.routing, [`${key}Level`]: e.target.value } }))}
                            className="w-full border rounded px-2 py-1.5 text-xs">
                ...
                <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50">
                  <button onClick={closeModal} className="px-4 py-2 text-sm text-gray-700 border rounded-lg hover:bg-gray-100">Cancel</button>
      ...
      {activeTab === "service-requests" && <RequestTypeConfig embedded />}
